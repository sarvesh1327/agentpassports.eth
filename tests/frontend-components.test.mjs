...
test("README documents release demo, architecture, env, and limitations", async () => {
  ...
  assert.match(source, /Visa metadata is generated by the app and pinned through Pinata/);
  assert.match(source, /curl -fsSL https:\/\/agentpassports\.eth\/install \| bash/);
  assert.match(source, /curl -fsSL https:\/\/raw\.githubusercontent\.com\/sarvesh1327\/agentpassports\.eth\/main\/scripts\/install-agentpassports\.sh \| bash/);
  assert.match(source, /agentpassports-create-key/);
  assert.match(source, /agentpassports-sign-intent --input build-task-intent\.json/);
  assert.match(source, /Private keys stay local/);
  assert.match(source, /MCP never receives private keys/);
});

test("home page renders the Register Agents marketing landing page with wallet-gated routes", async () => {
  await assertFile("scripts/install-agentpassports.sh");
  await assertFile("apps/web/app/install/route.ts");
  ...
  const installRouteSource = await readText("apps/web/app/install/route.ts");
  const installScript = await readText("scripts/install-agentpassports.sh");
  ...
  assert.match(landingSource, /Agent Skill Pack/);
  assert.match(landingSource, /curl -fsSL https:\/\/agentpassports\.eth\/install \| bash/);
  assert.match(landingSource, /Copy install command/);
  assert.match(landingSource, /agentpassports-create-key/);
  assert.match(landingSource, /Private key stays local/);
  assert.match(landingSource, /MCP stays thin/);
  assert.match(landingSource, /KeeperHub validates Visa/);
  assert.match(installRouteSource, /install-agentpassports\.sh/);
  assert.match(installRouteSource, /raw\.githubusercontent\.com\/sarvesh1327\/agentpassports\.eth/);
  assert.match(installScript, /AGENTPASSPORTS_HOME/);
  assert.match(installScript, /agentpassports-create-key/);
  assert.match(installScript, /agentpassports-sign-intent/);
  assert.match(installScript, /Does not read or write \.env files/);
  assert.match(installScript, /never touches env files/i);
  assert.doesNotMatch(installScript, />\s*\.env\b/);
  ...
  assert.match(styles, /\.landing-install-command-card\s*{/);
  assert.match(styles, /\.landing-safety-row\s*{/);
